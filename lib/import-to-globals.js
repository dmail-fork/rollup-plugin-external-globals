...
function analyzeExportNamed(node, code, names, tempNames) {
  if (
    node.declaration ||
    !node.source ||
    !names.hasOwnProperty(node.source.value)
  ) {
    return false;
  }
  for (const spec of node.specifiers) {
    const globalName = makeGlobalName(
      spec.local.name,
      names[node.source.value]
    );
    const legalName = /^[\w$]+$/.test(globalName)
      ? null
      : `_global_${makeLegalIdentifier(globalName)}`;
    if (!legalName) {
...
function importToGlobals({ ast, code, names }) {
  ...
  for (const node of ast.body) {
    if (node.type === "ImportDeclaration") {
      isTouched =
        analyzeImport(node, bindings, code, names, globals) || isTouched;
